...
- scope-decision: a feature the docs go back-and-forth on, where a ship/defer call is genuinely unresolved and the maintainer should decide
...
- Should those .tgz build artifacts be checked into git at all? Flag if they're stale/committed by accident.`,
  ...
  {
    key: 'tests',
    prompt: `DIMENSION: Test coverage & the Node-compat harness. You MAY run \`cargo test\` (you are the only dimension allowed to — others use the prebuilt binary). First check if a background run already finished: read /private/tmp/claude-501/-Users-user-Documents-projects-dun/174ff5c8-3443-4ea4-b868-f14566c104b7/tasks/bs0g548zm.output ; if empty, run \`cargo test --release 2>&1 | tail -40\` yourself and report pass/fail counts.
...
const SYNTH_SCHEMA = {
  ...
  properties: {
    ...
    one_liner: { type: 'string', description: 'one sentence the maintainer can read first.' },
    ...
    scope_decisions_for_colin: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['question', 'context', 'recommendation'], properties: { question: { type: 'string' }, context: { type: 'string' }, recommendation: { type: 'string' } } }, description: 'genuine ship/defer forks only the maintainer can resolve.' },
