...
**EXCEPTION — the file-tool safety does NOT cover raw git operations (learned 2026-06-12, the hard way).** The "Edit/Write reject stale edits" protection only guards the *file tools*. An agent that runs `git reset`/`git checkout`/`git stash` via Bash on a shared tree bypasses it entirely and WILL wipe a sibling agent's uncommitted edits. This actually happened: two agents editing the SAME `vendor/aube` submodule concurrently — one ran `git reset` twice and erased the other's WIP (it recovered by re-patching during a quiet window and committing immediately). So: **do NOT run two landing agents that EDIT THE SAME vendored submodule/tree concurrently** — serialize them, OR ensure they touch strictly disjoint files AND explicitly forbid destructive git ops in their prompts (no `reset`/`checkout`/`stash`; stage only their own files; commit fast to shrink the WIP window). Two agents editing *different* trees (one in `vendor/aube`, one in `crates/`) are fine. The general "parallelize hard" still holds — this is the one concrete carve-out where concurrent edits genuinely bite.
