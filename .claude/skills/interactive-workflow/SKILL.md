...
**DEFAULT TO DISPATCH — investigation is delegable work, and doing it in the foreground is the #1 recurring failure (the maintainer, repeatedly, "incredibly disappointed," 2026-06-13).** The reflex to kill: a problem appears (a red CI job, a bug, a "why does X happen", a provenance question, a log to read, several files to trace) and you start doing it YOURSELF, one foreground tool-call at a time. STOP. **Forensics, diagnosis, log-reading, code-tracing, repro, and fixes are exactly what sub-agents are for.** The moment a task is more than a single quick lookup, dispatch a sub-agent (or several) — model-tiered — instead of single-threading it in the foreground. The foreground is for ORCHESTRATION ONLY: synthesizing what agents return, making decisions, updating the tracker, and the final git that finalizes reviewed work. **The tell you're failing: you're on your 2nd+ foreground Bash/Read/grep of an *investigation* (not a quick orchestration check) — that work should be in a sub-agent's context, not yours.** When two+ problems are open, dispatch them as parallel lanes in ONE message — do not serialize what has no dependency. Keep several lanes busy by default; an idle fleet while you DIY is the anti-pattern. (This rule has had to be repeated many times — when in doubt, dispatch.)
...
**Tracker hygiene — it is a LIVING STATUS of CURRENT work, NOT a changelog (the maintainer, 2026-06-13, emphatic).** Do not accumulate chronological "update" entries, and do not keep finished `[x]` items with long post-mortems. When an item is DONE, **delete it** — snapshot the whole file to `todo-<ISO>.md` first, then rewrite lean (git history + the snapshot hold the past). Prefer DELETE over an ever-growing `## Archive` fold. Put STRUCTURED STATE (e.g. `autonomous_mode`, a heartbeat idle-counter) in the doc's **YAML front matter** — parsed by hooks — never narrated as a prose heading ("AUTONOMOUS MODE RE-ENABLED…" in the body is exactly the slop to avoid). Lean shape: front-matter (state) → how-to-use note → durable rules → in-flight dispatch ledger → open work → awaiting-human → durable state. Before adding a line, ask "is this CURRENT status?" — if it's history, it doesn't belong.

**Chain sub-agents — an auto-epilogue + a dispatch ledger so each agent's role in the broader plan survives compaction + parallel fan-out (the maintainer, 2026-06-13).** Two mechanisms: (1) a PreToolUse-on-`Agent` hook (`.claude/hooks/agent-dispatch.ts`) auto-appends an ORCHESTRATION EPILOGUE to every backgrounded dispatch, instructing the sub-agent to end with a `## Follow-ups` section — concrete follow-ups, a self-review rec if it built something substantial, a push-to-`main`+CI-watch rec if it touched code/tests, and the single most important next step (+ whether it needs the human). So agents always hand back the next links in the chain. (2) A **dispatch ledger** in the tracker (`agent | role | part-of-plan | ON RETURN → next step`): before dispatching step 1 of a major block, write the whole agent CHAIN into the ledger; on each return, look up its row to recall the role + next step. The ledger is what keeps the broader implementation plan from evaporating when context compacts or many agents run at once.
...
- **The heartbeat prompt must LEAD with a standing reminder to FAN OUT / PARALLELIZE / SYNC the tracker (the maintainer, 2026-06-13, emphatic).** Every re-armed heartbeat carries — verbatim, at the top — an instruction to: re-read this skill, re-read + update the tracker (`todo --not-done`), and check whether the fleet is thin (if open work and < ~4 agents running, fan out NOW in isolated clones, do NOT single-thread foreground forensics). The orchestrator drifts back into sequential foreground work between ticks; the human must NOT have to repeat "parallelize / check todo.md." Bake it into the heartbeat template so it self-reinforces each tick. (Burned 2026-06-13: single-threaded CI-log forensics with one workflow running while four failures sat un-fanned-out, until the maintainer said "YOU MUST ALWAYS BE PARALLELIZING AND CHECKING IN ON TODO.MD" then "the heartbeat should be reminding you.")
