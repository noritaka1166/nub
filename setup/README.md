...
| `registry-url` | — | Registry to set up for auth. Writes a temporary neutral `.npmrc` and wires auth to `env.NODE_AUTH_TOKEN`. |
...
Accepted for setup-node compatibility but **ignored in v1** (never errors): `token`, `check-latest`, `architecture`, `mirror`, `mirror-token`.
