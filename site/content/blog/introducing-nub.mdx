...
Nub maintains a content-addressed store on disk — each package version lands once, identified by content hash. A warm reinstall reuses that store and links files into `node_modules` with reflinks or hardlinks when the filesystem supports them. On a real [create-t3-app](https://create.t3.gg/) project that's about 2.5× faster than pnpm and faster than Bun:
...
The win is the warm materialization path: when the content-addressed store already holds a package, Nub writes a reflink (APFS/btrfs) or hardlink (ext4) into `node_modules` instead of copying bytes, so a warm reinstall is mostly filesystem metadata work. The pnpm and Bun installers use related store-and-link strategies; this benchmark measures Nub's current implementation of that path.
