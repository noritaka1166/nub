...
|---|---|---|
| Node 25.8 | 100% | 4,375 / 4,375 |
| **Nub** | **98.7%** | **4,318 / 4,375** |
| Deno 2.8 | 76.6% | 3,351 / 4,375 |
| Bun 1.3.14 | 40.1% | 1,756 / 4,375 |

*Deno's Node-compat suite, node-relative. Nub's gap is 57 documented deltas — module-hook and native-addon visibility, plus three intended divergences from default-on Web Storage and compile caching. [Reproduce →](https://github.com/nubjs/nub/tree/main/tests/cross-runtime)*
...
## `nub run` — a 20× faster `pnpm run`

A drop-in replacement for `npm run` and `pnpm run`, minus the ~300–350 ms Node bootstrap they pay on every invocation:
...
`pnpm` and `npm` are themselves Node.js programs: every `pnpm run <script>` boots a Node process, loads the package manager, parses its config, walks the workspace, and finally shells out to your script. That's ~300–350 ms of overhead before your script's first byte runs — and in a 30-package monorepo, `pnpm -r run build` pays it 30 times.

**Script-runner overhead** — a trivial `echo hi` script, so the figure is pure wrapper cost:
...
<Bench
  ...
  max={365}
  rows={[
    { cmd: 'nub run echo', ms: 13, us: true },
    { cmd: 'npm run echo', ms: 300, ratio: 23 },
    { cmd: 'pnpm run echo', ms: 350, ratio: 27 },
  ]}
/>
...
$ nub install   # yarn.lock → read and written back
```

There's no migration. `nub install` detects the lockfile your project already has — npm, pnpm, bun, or yarn — and writes the same format back, never a foreign file. Fresh projects get a standard `pnpm-lock.yaml`. And your package manager keeps working, side by side: run either tool, commit the same file, switch back any time.
