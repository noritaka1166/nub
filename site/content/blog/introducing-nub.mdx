...
$ nub run build            # script runner — 25× faster than pnpm run
$ nubx prisma generate     # bin runner — 20× faster than npx
$ nub watch src/server.ts  # watcher — driven by the dependency graph
$ nub install              # package manager — pnpm-compatible, your lockfile
$ nub node install 26      # version manager — installs Node on demand
```
...
Web Storage is native in Node 25+; on 22.4–24 Node hides it behind a flag, which Nub injects automatically — so `localStorage` works on every supported version. Node also leaves persistence broken by default: it silently fails unless you pass `--localstorage-file`. Nub defaults the storage file to `~/.cache/nub/webstorage/<workspace-hash>/localstorage.sqlite` — scoped to your workspace, so monorepo members share one store and it survives across runs. Your own `--localstorage-file`, set anywhere, always wins. The `sessionStorage` global is in-memory and per-process, by design.
...
The benchmarked script is identical on every row — the delta is pure wrapper overhead. Methodology: `hyperfine --warmup 3 --runs 100`, Apple M1 Max, Node 24.14.0 — the numbers are machine-specific, the ratios are the portable claim. Reproduce with [`benchmarks/`](https://github.com/nubjs/nub/tree/main/benchmarks).
...
## `nub install` — a 10× faster `pnpm install`
...
Second, on a pnpm project, support is complete: nub reads and honors every config surface pnpm does — `pnpm-workspace.yaml`, `.pnpmfile.cjs`, the `"pnpm"` package.json section, the `npm_config_*` environment — so the install it produces is one real pnpm accepts unchanged.

### Performance

A warm install points `node_modules` straight at one global store shared across every project, so it writes little more than symlinks. On a 1,168-package fixture that's about 10× faster than pnpm:
...
The win is the global virtual store: package files link into one machine-wide store instead of being copied per project, so a warm install is almost pure symlinks. Bun is fast too, but it still materializes every package into each project — that extra file work is the gap you see above.
...
- **Lifecycle scripts are deny-by-default.** A dependency's `postinstall` won't run unless you approve that package, so a compromised transitive dep can't execute code just because you installed it.
