...
Browser-standard WebSocket client. Available from Node 20.10+; Nub injects `--experimental-websocket` below 22.0, native above. Below 20.10 Node has no WebSocket client and Nub doesn't fake one.
...
Embedded SQLite. Available from Node 22.5+; Nub injects `--experimental-sqlite` until 22.13 where Node stabilized it. Below 22.5 it doesn't exist and Nub doesn't fake it.
...
A warm install symlinks `node_modules` into a global store shared across every project instead of copying package files, so almost no files are written. On a 1,168-package fixture that's about 10× faster than pnpm:
...
The win is the global virtual store: package files link into one machine-wide store instead of being copied per project, so a warm install mostly creates symlinks rather than copying files. Bun is fast too, but it still materializes every package into each project — that extra file work is the gap you see above.
