...
Bun and Deno proved there's enormous appetite for an all-in-one toolkit that replaces this pile. But neither has dented Node's production dominance — six years after Deno 1.0 and two and a half after Bun 1.0, neither is a first-class target on AWS, GCP, or Azure, and their Node.js conformance still trails the real thing by a wide margin. Both are from-scratch runtimes reimplementing Node's surface, and the gaps show up in production.

The "next-generation" runtimes share a fundamental flaw: **they're trying to replace Node.js.** Nub embraces it instead. It's a single Rust binary that transpiles your code and executes it on the stock `node` your project pins — so it passes the vast majority of Node's own test suite, because the runtime underneath *is* Node. It combines:

- **A file runner** — `nub index.ts`. Full TypeScript support (parity with Bun), running on the stock `node` binary.
- **A script runner** — `nub run build`. 25× faster than `pnpm run`.
- **A bin runner** — `nubx prisma generate`. 20× faster than `npx`.
...
- **A package manager** — `nub install`. The CLI surface is pnpm's, lockfile-compatible with any project — npm, pnpm, or bun.
...
Nub transpiles your code in memory with [oxc](https://oxc.rs) (compiled into a native Node addon) and runs the output on the stock `node` binary. There is no Nub runtime — just real Node, with enhancements. The transpile costs nothing measurable: running a TypeScript file adds negligible overhead over plain `node`, and runs about 2.9× faster than `tsx` — while supporting the non-erasable syntax Node's stripper rejects:
...
<Bench
  label="echo noop · pure shell · hyperfine, 100 runs"
  source="https://github.com/nubjs/nub/tree/main/benchmarks"
  accent="acid"
  max={356}
  rows={[
...
The benchmarked command is identical on every row — the delta is pure wrapper overhead. Methodology: `hyperfine --warmup 3 --runs 100`, Apple M1 Max, Node 24.14.0 — the numbers are machine-specific, the ratios are the portable claim. Reproduce with [`benchmarks/`](https://github.com/nubjs/nub/tree/main/benchmarks).
...
Everything your scripts depend on still works: the `npm_*` env vars, `pre` / `post` lifecycle hooks, and pnpm-style `node_modules/.bin` resolution in monorepos. The `.npmrc#script-shell` config key is honored.
...
Nub ships a full package manager, powered by the embedded [aube](https://github.com/jdx/aube) engine — built by [jdx](https://github.com/jdx), the creator of mise. The CLI surface is pnpm's; the lockfile is whatever your project already uses:
...
There's no migration — `nub install` detects the lockfile your project already has and never drops a foreign file. The npm, pnpm, and bun lockfiles round-trip — read and written back in their own format; yarn is read-only: nub installs from your `yarn.lock` but leaves it untouched, so commits stay clean and yarn stays the tool that writes it. Fresh projects get a standard `pnpm-lock.yaml`. And your package manager keeps working, side by side: run either tool, commit the same file, switch back any time.

What aube brings:

- **Speed** — a warm install points `node_modules` straight at one global store shared across every project, so it writes little more than symlinks. On a 1,168-package fixture that's about 10× faster than pnpm:

<Bench
  label="warm install · 1,168 deps · Linux · hyperfine"
  source="https://github.com/nubjs/nub/tree/main/tests/bench"
  accent="pink"
  max={1938}
  rows={[
...
/>
  ...
  The win is the global-virtual-store path, which is the default. Apps that need real files on disk — Next, Nuxt — materialize instead and match pnpm.

- **The tightest security defaults of any Node package manager** — lifecycle scripts wait for approval (with an opt-in build jail — the only one in the ecosystem), exotic transitive deps are blocked, and brand-new releases sit in a [24-hour cooling window](https://github.com/jdx/aube).
...
**No.** Your code runs in Node itself — once it's executing, V8 is V8. What's dramatically faster is the CLI surface: `nub run` is 25× faster than `pnpm run`, and `nubx` is 20× faster than `npx` on cold start, because those tools pay a full Node bootstrap to do what amounts to a script lookup, and Nub — a native Rust binary — doesn't.
...
Node 22.18+ runs `.ts` files by default — but it *strips* types, it doesn't transpile. Enums error. Namespace blocks error. Parameter properties error. JSX doesn't work. Decorator metadata doesn't work. Node 26 even removed `--experimental-transform-types`, the flag that used to handle enums — the Node TSC's posture is that full transpilation is userland's job.
