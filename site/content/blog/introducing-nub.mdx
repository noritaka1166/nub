...
- **A package manager** — `nub install`. pnpm-compatible, keeps the lockfile you already have.
...
## `nub node` — a Node version manager you should never need

Nub manages Node versions the way it manages everything else: implicitly. Every time it runs a file, it resolves the right Node for the project — from `.node-version`, `.nvmrc`, or `package.json#engines` — and if that version isn't installed, it's downloaded from nodejs.org (checksum-verified, cached under `~/.cache/nub`) and your code runs on it in the same breath. No `nvm use`, no prompt, no second step: the pin file *is* the consent. It's the [uv](https://docs.astral.sh/uv/) experience, for Node.
...
No pin? Nub runs the `node` already on your `PATH`, however you manage it — `nvm`, `mise`, `fnm`, `brew`. And for the rare day you want manual control, the explicit commands exist — no shell hooks, no `PATH` munging:
...
Nub ships a pnpm-compatible package manager, powered by the embedded aube engine — built in partnership with jdx.

There's no migration. `nub install` detects the lockfile your project already has and writes the same format back — never a foreign file, and `yarn.lock` is honored read-only rather than rewritten. Fresh projects get a standard `pnpm-lock.yaml`.
