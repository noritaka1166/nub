...
The "next-generation" runtimes share a fundamental flaw: **they're trying to replace Node.js.** Nub embraces it instead. It's a single Rust binary that transpiles your code and executes it on the stock `node` you already have installed — so it passes the vast majority of Node's own test suite, because the runtime underneath *is* Node. It combines:

- **A file runner** — `nub index.ts`. Full TypeScript support (parity with Bun), running on the stock `node` binary.
- **A script runner** — `nub run build`. ~7× faster than `pnpm run`.
...
## `nub run` — a 7× faster `pnpm run`

A drop-in replacement for `npm run` and `pnpm run`, minus the Node bootstrap they pay on every invocation:
...
**Script-runner overhead** — a trivial `noop` script, so the figure is pure wrapper cost:

<Bench
  label="run a package.json script · hyperfine, 100 runs"
  accent="acid"
  max={432}
  rows={[
    { cmd: 'nub run noop', ms: 60, us: true },
    { cmd: 'pnpm run noop', ms: 423, ratio: 7 },
    { cmd: 'corepack pnpm run noop', ms: 432, ratio: 7 },
  ]}
/>
...
There's no migration. `nub install` detects the lockfile your project already has and never drops a foreign file. npm, pnpm, and bun round-trip — read and written back in their own format. yarn is read-only: nub installs from your `yarn.lock` but leaves it untouched, so commits stay clean and yarn stays the tool that writes it. Fresh projects get a standard `pnpm-lock.yaml`. And your package manager keeps working, side by side: run either tool, commit the same file, switch back any time.
...
**No.** Your code runs in Node itself — once it's executing, V8 is V8. What's dramatically faster is the CLI surface: `nub run` is ~7× faster than `pnpm run`, and `nubx` beats `npx` by an order of magnitude on cold start, because those tools pay a full Node bootstrap to do what amounts to a script lookup, and Nub — a native Rust binary — doesn't.
