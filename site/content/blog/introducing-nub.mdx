...
Nub embraces it instead. It's a single Rust binary that transpiles your code and executes it on the stock `node` your project pins — so it passes Node's own test suite, because the runtime underneath *is* Node: 100% in `--node` passthrough mode and 99.3% augmented, over the full corpus with zero exclusions. The augmented gap is deliberate behavior — enabled features, suppressed experimental-warning noise, superseded TS handling — not breakage. It combines:
