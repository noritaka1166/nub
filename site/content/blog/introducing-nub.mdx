...
## `nub install`: a 3× faster `pnpm install`
...
Nub maintains a content-addressed store on disk — each package version lands once, identified by content hash. A warm reinstall reuses that store, reflink- or hardlink-copying files already on disk into `node_modules` instead of redownloading or recopying them. On a real [create-t3-app](https://create.t3.gg/) project that's about 3× faster than pnpm and faster than Bun:

<Bench
  label="warm install · create-t3-app · 222 deps · macOS · hyperfine"
  source="https://github.com/nubjs/nub/tree/main/tests/bench"
  accent="pink"
  max={4732}
  rows={[
    { cmd: 'nub install', ms: 974, us: true },
    { cmd: 'bun install', ms: 1498, ratio: 1.54 },
    { cmd: 'pnpm install', ms: 2743, ratio: 2.82 },
    { cmd: 'npm ci', ms: 4732, ratio: 4.86 },
  ]}
/>

The win is relinking: when the content-addressed store already holds a package, Nub writes a reflink (APFS/btrfs) or hardlink (ext4) into `node_modules` instead of copying bytes — so a warm reinstall is mostly an inode operation regardless of project size. Bun uses a similar store strategy, which is why it's fast too; Nub edges it out because the linking path avoids extra materialization work Bun still does per project.
