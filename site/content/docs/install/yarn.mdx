...
Nub reads the global `~/.yarnrc.yml` plus any project `.yarnrc.yml` files from the workspace root down to the current project directory; nearer project files win. The support is intentionally narrow: CA/proxy fields, `supportedArchitectures`, constraints, plugins, patch-folder config, and Yarn cache layout are still ignored.
