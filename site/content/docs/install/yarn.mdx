...
Project `.yarnrc.yml` outranks the global `~/.yarnrc.yml`, matching Yarn's rc-file precedence. The support is intentionally narrow: CA/proxy fields, `packageExtensions`, `supportedArchitectures`, constraints, plugins, patch-folder config, and Yarn cache layout are still ignored.
