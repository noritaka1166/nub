...
- **Partial `bunfig.toml` only.** Runtime/test/serve fields, security scanner config, cache/global-dir behavior, `cafile` / `ca`, and the rest of Bun's install config are not implemented. For registry/auth that must be shared across tools, `.npmrc` remains the most portable home:
