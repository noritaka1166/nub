...
<CompatTable
  rows={[
    ...
    { feature: <code>bunfig.toml</code>, status: 'partial', note: 'registry, scopes, linker only; Bun-incumbent projects only' },
...
- **Partial `bunfig.toml` only.** Runtime/test/serve fields, security scanner config, cache/global-dir behavior, and the rest of Bun's install config are not implemented. Custom CAs are supported, but through `.npmrc` rather than bunfig's own `cafile` key, which the install-only reader does not parse. The `.npmrc` `cafile` / `ca` keys (top-level and per-registry) work under every incumbent — see [Custom CAs](/docs/install/npm#custom-cas). For registry, auth, or TLS that must be shared across tools, `.npmrc` is the most portable home:
