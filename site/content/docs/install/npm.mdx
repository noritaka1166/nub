---
title: npm
description: Nub speaks npm's on-disk formats — the version-2 and version-3 lockfile round-trips byte-for-byte, and npm config is read across npm's builtin, global, user, and project scopes. The CLI is pnpm-shaped; the files are npm's.
---
...
Everything at those scopes works — default and scoped registries, auth tokens, TLS (`strict-ssl`, `cafile`), proxies, and install-shaping keys like `engine-strict` and `save-exact`. The `npm_config_*` env vars drive install behavior too. Precedence matches npm, highest first: CLI flags, then `npm_config_*`, then project, user, global, and builtin `.npmrc`.
