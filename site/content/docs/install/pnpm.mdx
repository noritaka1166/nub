...
Older formats — v6 (pnpm 8) and v5.4 (pnpm 7) — keep root dependencies under a top-level `dependencies:` map instead of v9's `importers:`. Nub's reader looks only at `importers:`, so it sees an empty project, treats the install as satisfied, and links nothing:
...
This is a silent no-op, not a clean error — migrate by running a one-time `pnpm install` (or `pnpm dedupe`) under pnpm 9+ to upgrade the lockfile to v9 first.
...
- **Lockfile is v9-only.** v6 (pnpm 8) and v5.4 (pnpm 7) are not parsed — Nub silently treats them as an empty/satisfied project and links nothing. Migrate with pnpm 9+ first.
