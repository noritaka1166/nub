...
<CompatTable
  ...
  rows={[
    ...
    { feature: <>dependency verbs</>, status: 'partial', note: 'Core verbs are wired: install, ci, add, remove, update, import, dedupe, prune, rebuild, fetch, link, unlink, patch, patch-commit, patch-remove, approve-builds, ignored-builds, dlx, and create. deploy, clean, purge, and the recursive meta-verb are not.' },
    { feature: <>workspace selectors</>, status: 'partial', note: 'Workspace-aware dependency verbs accept -r, --filter, --filter-prod, --fail-if-no-match, and --include-workspace-root. Use those instead of the recursive meta-verb.' },
