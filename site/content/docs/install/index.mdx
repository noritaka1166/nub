...
- **Package fields:** `workspaces`, top-level `overrides` and `resolutions`, `catalog` / `catalogs`, top-level `patchedDependencies`, top-level `allowBuilds`, `dependenciesMeta.injected`, `engines.node`, and lifecycle `scripts`.
