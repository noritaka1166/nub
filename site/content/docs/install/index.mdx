...
When the global store already holds every package, a warm install is mostly links into that shared store — no re-download, no re-copy — so it runs near-instantly, and offline.
