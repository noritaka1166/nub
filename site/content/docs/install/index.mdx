...
Nub ships a **deny-by-default** install posture. Beyond the packages you approve explicitly, a curated set of well-known packages — aube's default-trust floor, off in standalone aube and flipped on under Nub — may build without approval, but only when **all three** gates hold at once:
...
Explicit decisions outrank the floor in both directions. A package you approve through `pnpm.onlyBuiltDependencies`, `pnpm.allowBuilds`, Bun's `trustedDependencies`, or `nub approve-builds` builds regardless; an explicit denial (`allowBuilds: { pkg: false }`, `neverBuiltDependencies`) always wins.
