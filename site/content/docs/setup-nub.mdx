...
The setup-node inputs `token`, `check-latest`, `architecture`, `mirror`, and `mirror-token` are accepted and ignored — present so a swap never errors, with no effect. A `version` input survives as a deprecated alias for `nub-version` and emits a warning.
