...
The action installs the `nub` CLI and eagerly provisions the project's pinned Node — downloaded and cached during setup, so it is ready before any of your steps run. The pin is resolved from the project files on disk (`devEngines.runtime` → `.node-version` → `.nvmrc` → `engines.node`), so `actions/checkout` must run first.
