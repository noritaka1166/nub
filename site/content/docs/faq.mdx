...
Nub is the inverse bet. The compatibility surface stays Node's — your code is plain Node code, no `globalThis.nub`, no `nub:*` module namespace, no `@nub/*` npm scope, no `NUB_*` environment variables in your code or config. The augmentations Nub applies are all things that work on plain Node with the matching `module.register()` / `--import` / npm addon. Most of Bun's "all-in-one" pitch is now either native to Node or implementable on top of it; Nub assembles the rest.
