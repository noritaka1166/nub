...
No — Nub *is* Node. Your code is transpiled and executed by the stock `node` binary; there's no Nub runtime to discover and nothing reimplemented. Nub is a layer of developer-experience defaults on top of Node, built entirely on Node's own extension surfaces. None of this is a criticism of Node — its caution about breaking changes is the right caution, and that same caution is exactly why opinionated modern defaults belong in a layer on top rather than in core.
...
- **`WebSocket`** — Browser-standard WebSocket client. On by default since Node 22.0 (flag-gated on 20.10–21.x, where Nub injects `--experimental-websocket`); Nub polyfills via [`ws`](https://github.com/websockets/ws) on older Node
