...
`nub run` is a drop-in for `pnpm run` — every flag carries over with the same spelling and semantics, including the recursive and workspace-filter ones a real monorepo leans on. It is equally a drop-in for `npm run`. Existing invocations work unchanged:
...
A few minor behavioral notes: `--report-summary` streams results live via `--reporter=ndjson` rather than writing a static file; `--if-present` is workspace-scoped (skips members missing the script during a recursive run); and the legacy npm knobs `--scripts-prepend-node-path`, `--unsafe-perm`, and `--npm-path` are not needed and not accepted.
