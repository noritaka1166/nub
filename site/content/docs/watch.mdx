...
Nub preserves terminal output by default and prints a restart banner, rather than clearing the screen — so the previous run's error message stays on screen.
