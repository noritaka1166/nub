...
| Nub | Replaces |
|---|---|
| `nub <file>` | `node`, `tsx`, `ts-node`, `dotenv-cli`, `node --env-file`, `deno run`, `bun run` |
| `nub run <script>` | `npm run`, `pnpm run`, `yarn run`, `bun run`, `pnpm -r` |
| `nubx` / `nub dlx` | `npx`, `pnpm dlx`, `pnpm exec`, `yarn dlx`, `bunx` |
| `nub install` | `npm`, `pnpm`, `yarn`, `bun` |
| `nub watch` | `nodemon`, `node --watch`, `tsx watch`, `bun --watch` |
| `nub node` | `nvm`, `fnm`, `n`, `volta`, `asdf`, `corepack` |
