...
const PROMPT =
  'Read https://nubjs.com/start.md to learn nub’s full surface. Then install the nub skill — run `nub agent skill`, or fetch https://nubjs.com/skill.md — into wherever this project keeps agent skills or rules, following the project’s conventions (for example .claude/skills, .cursor/rules, or your agent’s equivalent). Next, look at this project’s dependencies and scripts and explain to me how and when to use nub here: run files with `nub <file>`, package.json scripts with `nub run`, local CLIs with `nubx`, and `nub --node` for plain, unaugmented Node. Then find any dependencies or devDependencies that nub makes redundant (for example tsx, ts-node, dotenv, nodemon, tsconfig-paths) and offer to migrate the project off them onto nub. There’s no need to switch off the existing package manager — nub respects its lockfile bidirectionally, so nub and the current package manager work interchangeably.';
